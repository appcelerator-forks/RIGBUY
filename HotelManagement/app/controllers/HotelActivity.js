...
if (OS_IOS) {
	...
	$.HotelActivity.title = Alloy.Globals.langConvert("hotel_activity");
	if (lang == "ar") {
		$.HotelActivity.leftNavButton = notificationBtn;
		$.HotelActivity.rightNavButton = btnVW;

	} else {
		$.HotelActivity.leftNavButton = btnVW;
		$.HotelActivity.rightNavButton = notificationBtn;
	}
...
function renderHomeGrid(sample_data) {
	...
	for (var x = 0; x < sample_data.length; x++) {
		...
		if (Alloy.Globals.languageAr) {
			if (x == 4) {
				view.backgroundColor = "white";

			} else {
				var img = Ti.UI.createImageView({
					image : "http://myhotelsapp.com/hotel/uploads/activity_type/"+sample_data[x].image,
					width : Ti.UI.FILL,
					height : Ti.UI.FILL
				});
				view.add(img);
	...
	$.fg.setOnItemClick(function(e) {
		...
		HotelActivityCategoryService(e.source.data.id);
		// var mycart = Alloy.createController("HotelActivityCategory").getView();
		// Alloy.Globals.navWin.openWindow(mycart);
		// mycart.oldWin = $.HotelActivity;
		// Alloy.Globals.currentWindow = mycart;
...
function HotelActivityService() {

	if (Ti.Network.online) {
		Alloy.Globals.LoadingScreen.open();
		Communicator.get("http://myhotelsapp.com/api/api.php?action=GetActivityType&" + "language=" + Ti.App.Properties.getString("locale") + "&hotel_id=" + Ti.App.Properties.getString("hotel_id"), HotelActivityServiceCallback);
...
function HotelActivityServiceCallback(e) {
...
function openFunc(e) {
	HotelActivityService();
}

function HotelActivityCategoryService(menu_id) {

	if (Ti.Network.online) {
		Alloy.Globals.LoadingScreen.open();
		Communicator.get("http://myhotelsapp.com/api/api.php?action=GetHotelActivity&" + "language=" + Ti.App.Properties.getString("locale") + "&hotel_id=" + Ti.App.Properties.getString("hotel_id") + "&activity_id=" + menu_id, HotelActivityCategoryServiceCallback);
		Ti.API.info('URL : ' + "http://myhotelsapp.com/api/api.php?action=GetHotelActivity&" + "language=" + Ti.App.Properties.getString("locale") + "&hotel_id=" + Ti.App.Properties.getString("hotel_id") + "&menu_id=" + menu_id);
	} else {
...
function HotelActivityCategoryServiceCallback(e) {
	Ti.API.info("HotelActivityCategoryService Callback response : " + JSON.stringify(e));
	if (e.success) {
		try {
			// Ti.API.info('response ' + e.response);
			var response = JSON.parse(e.response);
			...
			if (response != null) {
				...
				if (response.success == '1') {
					var mycart = Alloy.createController("HotelActivityCategory", response["Food Menu"]).getView();
					Alloy.Globals.navWin.openWindow(mycart);
					mycart.oldWin = $.HotelActivity;
					Alloy.Globals.currentWindow = mycart;
				} else {
